...
The public publication snapshot is available at:
\begin{center}
\small\url{https://github.com/inlanger/esp32-p4-inflect-tts}
\end{center}
The immutable snapshot used by this paper is commit
...
\end{center}
It contains the manuscript source, rendered PDF, metadata, and arXiv upload
archive. Firmware, runtime patches, model packages, and listening artifacts
are still being consolidated while the separate decoder-training experiment
continues; the public repository records this release boundary explicitly.

The working experiment repository retains source changes, exporters, test
scripts, firmware, model packages, board logs, JSON summaries, and listening
WAVs. Its report records hypotheses, raw results, failures, and decisions in
chronological order. Each logically complete change is a separate Git commit.
